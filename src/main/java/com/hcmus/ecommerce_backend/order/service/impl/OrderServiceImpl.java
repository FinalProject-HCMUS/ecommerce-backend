...
public class OrderServiceImpl implements OrderService {
    ...
    public List<OrderResponse> getAllOrders() {
        log.info("OrderServiceImpl | getAllOrders | Retrieving all orders");
        try {
            List<OrderResponse> orders = orderRepository.findAll().stream()
                    .map(orderMapper::toResponse)
                    .collect(Collectors.toList());
            log.info("OrderServiceImpl | getAllOrders | Found {} orders", orders.size());
            return orders;
        } catch (DataAccessException e) {
            log.error("OrderServiceImpl | getAllOrders | Error retrieving orders: {}", e.getMessage(), e);
            return Collections.emptyList();
        } catch (Exception e) {
            log.error("OrderServiceImpl | getAllOrders | Unexpected error: {}", e.getMessage(), e);
            throw e;
        }
    }
    ...
    public List<OrderResponse> getOrdersByCustomerId(String customerId) {
        ...
        try {
            List<OrderResponse> orders = orderRepository.findByCustomerId(customerId).stream()
                    .map(orderMapper::toResponse)
                    .collect(Collectors.toList());
            log.info("OrderServiceImpl | getOrdersByCustomerId | Found {} orders for customer {}", orders.size(), customerId);
            return orders;
        } catch (DataAccessException e) {
            log.error("OrderServiceImpl | getOrdersByCustomerId | Database error for customerId {}: {}", customerId, e.getMessage(), e);
            return Collections.emptyList();
        } catch (Exception e) {
            log.error("OrderServiceImpl | getOrdersByCustomerId | Unexpected error for customerId {}: {}", customerId, e.getMessage(), e);
            throw e;
        }
    }
