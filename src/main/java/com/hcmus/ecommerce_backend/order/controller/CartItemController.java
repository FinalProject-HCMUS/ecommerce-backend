...
public class CartItemController {
    ...
    @Operation(summary = "Get cart items by order ID", description = "Retrieves cart items for a specific order")
    @ApiResponse(responseCode = "200", description = "Successfully retrieved order's cart items")
    @GetMapping("/order/{orderId}")
    public ResponseEntity<CustomResponse<List<CartItemResponse>>> getCartItemsByOrderId(
            @Parameter(description = "ID of the order", required = true)
            @PathVariable String orderId) {
        log.info("CartItemController | getCartItemsByOrderId | orderId: {}", orderId);
        List<CartItemResponse> cartItems = cartItemService.getCartItemsByOrderId(orderId);
        return ResponseEntity.ok(CustomResponse.successOf(cartItems));
    }
